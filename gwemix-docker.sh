...
shutdown() {
    ...
    /data/wemix/bin/gwemix-docker.sh stop
    exit 0
}

# [NEW] Trap SIGTERM and SIGINT
...
function start ()
{
    ...
    *)
    ...
    # [NEW] Start gwemix with data directory and options, redirect all output to both the console and logrot for log management
    # 로그 로테이션 백그라운드 실행 후 gwemix 포그라운드에서 실행
    ...
    #    tee >( "${d}/bin/logrot" "${d}/logs/log" "${LOG_FILESIZE}" "${LOG_FILECOUNT}" )
